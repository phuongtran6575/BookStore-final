...
@router.get("/")
async def get_book_categories(product_id: UUID | str, session: sessionDepends):
    try:
...
async def remove_category_from_book(product_id: UUID | str, category_id: UUID | str, session: sessionDepends):
    try:
        book_uuid = to_uuid(product_id)
